...
import (
	...
	"github.com/cedar2025/xboard-node/internal/deviceclaim"
...
func (s *Service) initialSetup(ctx context.Context) error {
	...
	if s.cfg.DeviceClaim.Enabled {
		if s.kernel.Name() != "sing-box" {
			return fmt.Errorf("device claim store requires sing-box kernel, got %s", s.kernel.Name())
		}
		store, err := deviceclaim.NewRedisStore(s.cfg.DeviceClaim)
		if err != nil {
			return fmt.Errorf("device claim store: %w", err)
		}
		s.kernel.SetDeviceClaimStore(store)
		nlog.Core().Info("shared device claim store enabled", "type", s.cfg.DeviceClaim.Type, "addr", s.cfg.DeviceClaim.Addr)
	}
