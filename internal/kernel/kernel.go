...
// DeviceClaimStore is a shared, atomic admission source for cross-node device
// limits. Implementations must make Claim and Release atomic across all nodes.
//
// Claim is called before a new connection is forwarded. It must allow an
// already-owned same device, allow a new device only when the user's current
// device count is below limit, and reject without replacing existing owners
// otherwise. ownerToken is unique per admitted connection and is later passed
// to Release.
type DeviceClaimStore interface {
	Claim(ctx context.Context, userUUID, deviceID, ownerToken string, limit int) (DeviceClaim, error)
	Release(ctx context.Context, userUUID, deviceID, ownerToken string) error
	Refresh(ctx context.Context, userUUID, deviceID, ownerToken string) error
}
...
type Kernel interface {
	...
	// SetDeviceClaimStore configures a shared atomic admission store. When set,
	// kernels that support it must claim before forwarding and release on close.
	SetDeviceClaimStore(store DeviceClaimStore)
