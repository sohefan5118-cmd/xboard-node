...
type SingBox struct {
	...
	// deviceClaimStore is an optional shared atomic admission source for
	// cross-node device limits. Set once and forwarded to every new ConnTracker.
	deviceClaimStore kernel.DeviceClaimStore
...
func (s *SingBox) Start(nodeConfig *model.NodeSpec, users []model.UserSpec, tls kernel.TLSCert) error {
	...
	if s.deviceClaimStore != nil {
		s.connTracker.SetDeviceClaimStore(s.deviceClaimStore)
	}
...
// SetDeviceClaimStore configures shared atomic cross-node device admission.
func (s *SingBox) SetDeviceClaimStore(store kernel.DeviceClaimStore) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.deviceClaimStore = store
	if s.connTracker != nil {
		s.connTracker.SetDeviceClaimStore(store)
	}
}
