...
type ConnTracker struct {
	...
	// deviceClaimStore is a shared atomic admission source. When configured it
	// replaces local/global snapshot gate-keeping for new device admission.
	deviceClaimStore atomic.Pointer[kernel.DeviceClaimStore]
...
// SetDeviceClaimStore configures a shared atomic admission source.
func (t *ConnTracker) SetDeviceClaimStore(store kernel.DeviceClaimStore) {
	t.deviceClaimStore.Store(&store)
}
...
func (t *ConnTracker) RoutedConnection(
	...
	var claim *kernel.DeviceClaim

	// Device limit gate-keeping. Prefer shared atomic claim store when present;
	// fall back to legacy local/global snapshot gate only when not configured.
	if dlf := t.deviceLimitFunc.Load(); dlf != nil {
		if limit, hasLimit := (*dlf)(uuid); hasLimit {
			if storep := t.deviceClaimStore.Load(); storep != nil && *storep != nil {
				owner := newOwnerToken()
				c, err := (*storep).Claim(ctx, uuid, sourceIP, owner, limit)
				if err != nil || !c.Allowed {
					nlog.Core().Info("singbox: device claim rejected connection",
						"user", uuid, "ip", sourceIP, "limit", limit, "error", err)
					conn.Close()
					return conn
				}
				claim = &c
			} else if t.checkDeviceGate(us, uid, sourceIP, limit) {
	...
	return &trackedConn{
		...
		userUUID:  uuid,
		claim:     claim,
		claimDone: startClaimRefresh(claim, &t.deviceClaimStore, uuid, sourceIP),
		limiter:   lim,
		ctx:       ctx,
	}
}
	...
	if dlf := t.deviceLimitFunc.Load(); dlf != nil {
		if limit, hasLimit := (*dlf)(uuid); hasLimit {
			if storep := t.deviceClaimStore.Load(); storep != nil && *storep != nil {
				...
				c, err := (*storep).Claim(ctx, uuid, sourceIP, owner, limit)
				if err != nil || !c.Allowed {
					nlog.Core().Info("singbox: device claim rejected UDP connection",
						"user", uuid, "ip", sourceIP, "limit", limit, "error", err)
					conn.Close()
					return conn
				}
...
// startClaimRefresh keeps an admitted owner alive while a connection is idle.
// The store TTL is only a crash-recovery guard; it must not expire a healthy
// long-lived connection merely because it has no traffic.
func startClaimRefresh(claim *kernel.DeviceClaim, store *atomic.Pointer[kernel.DeviceClaimStore], userUUID, deviceID string) chan struct{} {
	if claim == nil {
		return nil
	}
	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				storep := store.Load()
				if storep == nil || *storep == nil {
					continue
				}
				ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
				if err := (*storep).Refresh(ctx, userUUID, deviceID, claim.OwnerToken); err != nil {
					nlog.Core().Warn("singbox: device claim refresh failed", "user", userUUID, "ip", deviceID, "error", err)
				}
				cancel()
			case <-done:
				return
...
func newOwnerToken() string {
	...
	if _, err := rand.Read(b[:]); err != nil {
		return "owner-" + formatInt36(time.Now().UnixNano())
	}
...
func (c *trackedConn) releaseClaim() {
	if c.claim == nil {
		return
	}
	if storep := c.tracker.deviceClaimStore.Load(); storep != nil && *storep != nil {
		ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
		defer cancel()
		if err := (*storep).Release(ctx, c.userUUID, c.sourceIP, c.claim.OwnerToken); err != nil {
			nlog.Core().Warn("singbox: device claim release failed", "user", c.userUUID, "ip", c.sourceIP, "error", err)
		}
	}
}
...
func (c *trackedPacketConn) releaseClaim() {
	if c.claim == nil {
		return
	}
	if storep := c.tracker.deviceClaimStore.Load(); storep != nil && *storep != nil {
