...
func (s *testClaimStore) Claim(ctx context.Context, userUUID, deviceID, ownerToken string, limit int) (kernel.DeviceClaim, error) {
	_ = ctx
	s.claims = append(s.claims, userUUID+"/"+deviceID)
	if deviceID == "2.2.2.2" {
		return kernel.DeviceClaim{Allowed: false}, nil
	}
	return kernel.DeviceClaim{Allowed: true, OwnerToken: ownerToken}, nil
}

func (s *testClaimStore) Release(ctx context.Context, userUUID, deviceID, ownerToken string) error {
	_, _ = ctx, ownerToken
	s.releases = append(s.releases, userUUID+"/"+deviceID)
	return nil
}

func (s *testClaimStore) Refresh(ctx context.Context, userUUID, deviceID, ownerToken string) error {
	_, _, _, _ = ctx, userUUID, deviceID, ownerToken
	return nil
}

func TestConnTrackerUsesSharedDeviceClaimBeforeForwarding(t *testing.T) {
	tracker := NewConnTracker(0)
	tracker.SetUserMap(map[string]int{"uuid-1": 1})
	tracker.SetDeviceLimitFunc(func(uuid string) (int, bool) {
		if uuid != "uuid-1" {
	...
	store := &testClaimStore{}
	tracker.SetDeviceClaimStore(store)

	first := &testConn{}
	wrapped1 := tracker.RoutedConnection(context.Background(), first, testInboundContext("uuid-1", "1.1.1.1"), nil, nil)
	if wrapped1 == first {
		t.Fatal("expected admitted connection to be wrapped")
	}
	...
	if !second.closed {
		t.Fatal("expected rejected connection to be closed before forwarding")
	}

	if len(store.claims) != 2 {
		t.Fatalf("claims = %v, want two pre-forward claim attempts", store.claims)
	}
	if err := wrapped1.Close(); err != nil {
		t.Fatalf("Close() error = %v", err)
	}
	if len(store.releases) != 1 || store.releases[0] != "uuid-1/1.1.1.1" {
		t.Fatalf("releases = %v, want only admitted owner released", store.releases)
	}
}
