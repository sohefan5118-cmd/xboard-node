...
func TestLoad_DeviceClaimEnvironmentOverrides(t *testing.T) {
	path := writeTemp(t, `
panel:
  url: "https://panel.example.com"
  token: "tok"
...
`)
	t.Setenv("DEVICE_CLAIM_ENABLED", "true")
	t.Setenv("DEVICE_CLAIM_ADDR", "redis.internal:6379")
	t.Setenv("DEVICE_CLAIM_PASSWORD", "env-password")
	t.Setenv("DEVICE_CLAIM_DB", "2")
	t.Setenv("DEVICE_CLAIM_TTL", "600")

	cfg, err := Load(path)
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
	if !cfg.DeviceClaim.Enabled || cfg.DeviceClaim.Addr != "redis.internal:6379" {
		t.Fatalf("device claim override not applied: %+v", cfg.DeviceClaim)
	}
	if cfg.DeviceClaim.Password != "env-password" || cfg.DeviceClaim.DB != 2 || cfg.DeviceClaim.TTL != 600 {
		t.Fatalf("device claim values not overridden: %+v", cfg.DeviceClaim)
	}
}
