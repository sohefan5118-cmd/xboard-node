...
type Config struct {
	...
	WS          WSConfig          `yaml:"ws"`
	DeviceClaim DeviceClaimConfig `yaml:"device_claim"`
...
// DeviceClaimConfig enables a shared atomic admission store for cross-node
// device-limit enforcement. When enabled, a node must CLAIM before forwarding
// a new connection and RELEASE with the same owner token on close.
type DeviceClaimConfig struct {
	Enabled  bool   `yaml:"enabled"`
	Type     string `yaml:"type"` // currently "redis"
	Addr     string `yaml:"addr"` // host:port, default 127.0.0.1:6379
	Password string `yaml:"password,omitempty"`
	DB       int    `yaml:"db,omitempty"`
	Prefix   string `yaml:"prefix,omitempty"`
	TTL      int    `yaml:"ttl,omitempty"` // seconds, default 300
}
...
func (c *Config) applyEnvOverrides() {
	...
	// Shared device-claim store. Keep the password environment-only in
	// container/systemd deployments so it does not need to be written to YAML.
	if v := envFirst("DEVICE_CLAIM_ENABLED"); v != "" {
		if enabled, err := strconv.ParseBool(v); err == nil {
			c.DeviceClaim.Enabled = enabled
		}
	}
	if v := envFirst("DEVICE_CLAIM_TYPE"); v != "" {
		c.DeviceClaim.Type = v
	}
	if v := envFirst("DEVICE_CLAIM_ADDR"); v != "" {
		c.DeviceClaim.Addr = v
	}
	if v := envFirst("DEVICE_CLAIM_PASSWORD"); v != "" {
		c.DeviceClaim.Password = v
	}
	if v := envFirst("DEVICE_CLAIM_DB"); v != "" {
		if db, err := strconv.Atoi(v); err == nil {
			c.DeviceClaim.DB = db
		}
	}
	if v := envFirst("DEVICE_CLAIM_PREFIX"); v != "" {
		c.DeviceClaim.Prefix = v
	}
	if v := envFirst("DEVICE_CLAIM_TTL"); v != "" {
		if ttl, err := strconv.Atoi(v); err == nil {
			c.DeviceClaim.TTL = ttl
		}
	}
...
func (c *Config) inheritFrom(parent *Config) {
	...
	// Shared device claim defaults
	if !c.DeviceClaim.Enabled {
		c.DeviceClaim.Enabled = parent.DeviceClaim.Enabled
	}
	if c.DeviceClaim.Type == "" {
		c.DeviceClaim.Type = parent.DeviceClaim.Type
	}
	if c.DeviceClaim.Addr == "" {
		c.DeviceClaim.Addr = parent.DeviceClaim.Addr
	}
	if c.DeviceClaim.Password == "" {
		c.DeviceClaim.Password = parent.DeviceClaim.Password
	}
	if c.DeviceClaim.DB == 0 {
		c.DeviceClaim.DB = parent.DeviceClaim.DB
	}
	if c.DeviceClaim.Prefix == "" {
		c.DeviceClaim.Prefix = parent.DeviceClaim.Prefix
	}
	if c.DeviceClaim.TTL == 0 {
		c.DeviceClaim.TTL = parent.DeviceClaim.TTL
	}
	// WS
...
func (c *Config) setDefaultsFrom(baseDir string) {
	...
	if c.DeviceClaim.Enabled {
		if c.DeviceClaim.Type == "" {
			c.DeviceClaim.Type = "redis"
		}
		if c.DeviceClaim.Addr == "" {
			c.DeviceClaim.Addr = "127.0.0.1:6379"
		}
		if c.DeviceClaim.Prefix == "" {
			c.DeviceClaim.Prefix = "xboard:device-claim"
		}
		if c.DeviceClaim.TTL == 0 {
			c.DeviceClaim.TTL = 300
		}
	}
}
